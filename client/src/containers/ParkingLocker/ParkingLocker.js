...
const ParkingLocker = () => {
  ...
  const enrichPropertyDetailsWithUserEmails = async (propertyDetails) => {
    const parkingSpotsWithOwnerEmail = await Promise.all(
      propertyDetails.parkingSpots.map(async (spot) => {
        console.log('THIS IS THE SPOT OWNER ID:')
        console.log(spot.ownerId)
        var ownerEmail = ''
        if (spot.ownerId === null) {
          ownerEmail = ''
        } else {
          ownerEmail = await fetchUserEmailById(spot.ownerId)
        }
    ...
    const lockersWithOwnerEmail = await Promise.all(
      propertyDetails.lockers.map(async (locker) => {
        ...
        if (locker.ownerId === null) {
          ...
          ownerEmail = await fetchUserEmailById(locker.ownerId)
        }
  ...
  useEffect(() => {
    const fetchPropertyDetails = async () => {
      ...
      const response = await fetch(
        `http://localhost:5127/api/properties/${selectedPropertyId}`,
        { method: 'GET', headers }
      )
  ...
  const addParkingSpot = async (e) => {
    ...
    try {
      ...
      updatePropertyMutation.mutate(
        { type: 'parkingSpot', newData: parkingSpotWithOwner },
        {
          onSuccess: () => {
  ...
  const addLocker = async (e) => {
    ...
    try {
      ...
      updatePropertyMutation.mutate(
        { type: 'locker', newData: lockerWithOwner },
        {
          onSuccess: () => {
            window.location.reload()
          },
